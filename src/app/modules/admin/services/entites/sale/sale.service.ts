...
export class SaleService extends CrudServices {
  ...
  public invoiceCreate(data: any): Observable<any> {
    return this.http.post(`api/${this.entity}/create`, data);
  }


  public getPaymentMethods(): Observable<any> {
    return this.http.get(`${environment.AUTH_URL}/api/payment-methods`);
  }


  public getInvoiceDetail(option: any): Observable<any> {
    return this.http.get(`${environment.AUTH_URL}/api/${this.entity}/detail?invoice_id=${option.id}`);
  }
